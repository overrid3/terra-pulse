...
- **State machine guards** in `ServiceOrderStateMachine` are the source of truth for service-order transitions. The normal flow is `REQUESTED → QUOTED → APPROVED → SCHEDULED → IN_PROGRESS → COMPLETED`. The `SCHEDULED` transition requires a non-null mechanic, `scheduled_start_at`, and `scheduled_end_at`, and enforces a non-overlapping window for that mechanic. Override endpoint (`POST /service-orders/{id}/override-state`) is the only legitimate bypass and only allows `CANCELLED` / `REQUESTED` targets.
