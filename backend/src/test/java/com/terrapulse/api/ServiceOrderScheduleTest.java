...
/**
 * Integration tests for POST /api/service-orders/{id}/schedule.
 *
...
class ServiceOrderScheduleTest {
    ...
    /**
     * Persists a ServiceOrder in the given state with scheduledStartAt/EndAt set
     * (required by the SCHEDULED state guard in the state machine).
     */
    @Transactional(jakarta.transaction.Transactional.TxType.REQUIRES_NEW)
