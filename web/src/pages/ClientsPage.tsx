  ...
  return (
    <main className="page-grid two-col">
      <section className="panel">
        <h2>{t("clients.pageTitle", { count: qc.data?.length ?? 0 })}</h2>
        <table className="data-table">
          <thead>
            <tr>
              <th>{t("clients.columnName")}</th>
              <th>{t("clients.columnEmail")}</th>
              <th>{t("clients.columnVat")}</th>
              <th>{t("clients.columnCity")}</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {qc.data?.map((c) => (
              <tr key={c.id} className={editing?.id === c.id ? "selected" : ""}>
                ...
                <td className="mono">{c.vatNumber ?? t("common.dash")}</td>
                <td>{c.city ?? t("common.dash")}</td>
                <td className="row-actions">
                  <button onClick={() => loadForEdit(c)}>{t("common.edit")}</button>
                  <button className="danger" onClick={() => confirm(t("common.deleteConfirm", { label: c.name })) && deleteMut.mutate(c.id)}>{t("common.delete")}</button>
                </td>
              </tr>
            ))}
            {qc.data?.length === 0 && <tr><td colSpan={5} className="muted">{t("clients.noClients")}</td></tr>}
          </tbody>
        </table>
      </section>
